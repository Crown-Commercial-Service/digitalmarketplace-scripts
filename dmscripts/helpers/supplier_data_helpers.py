...
def unsuspend_suspended_supplier_services(record, suspending_user, client, logger, dry_run):
    """
    This method checks if the supplier has services suspended by the suspending_user, if so it un-suspends them. It
    returns None.
    ...
    # Suspend all services for supplier (the API will de-index the services from search results)
    logger.info(
