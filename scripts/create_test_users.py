...
def ensure_user_exists_for_supplier(data_api_client, supplier_id):
    try:
        user = next(data_api_client.find_users_iter(supplier_id=supplier_id))
    except StopIteration:
        data_api_client.create_user({
            "emailAddress": f"{supplier_id}@example.com",
            "name": "Test",
...
for duns_number in range(123456787, 123456790):
    ...
    user = ensure_user_exists_for_supplier(data_api_client, supplier['id'])
    print(f"User: {user['emailAddress']}")
