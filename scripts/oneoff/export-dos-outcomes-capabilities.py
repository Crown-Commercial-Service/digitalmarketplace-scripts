...
For a DOS-type framework this will export all published "digital-outcomes" services and details on the types
of outcome the supplier provides.

It is based of scripts/export-dos-outcomes.py

Usage:
    scripts/oneoff/export-dos-outcomes-capabilities.py <stage> <framework_slug> <content_path> [options]
...
def make_row(capabilities):
    def inner(record):
        row = [
            ("supplier_id", record["supplier_id"]),
        ]
        return row + make_fields_from_content_questions(capabilities, record)
...
if __name__ == '__main__':
    ...
    write_csv_with_make_row(
        suppliers,
        make_row(capabilities),
        os.path.join(OUTPUT_DIR, "digital-outcomes-suppliers-capabilities.csv"),
